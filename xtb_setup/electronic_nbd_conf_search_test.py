#!/home/elholm/anaconda3/bin/python
import sys
import pandas as pd
import copy
...
def gs_mogens(name, smi, chrg, mult, cps):
    ...
    for i, comp in enumerate([(reac_mol, "_r"), (prod_mol, "_p")]):
        ...
        if p_r == '_r':
            try:
                print("starting reac conf search")
...
if __name__ == '__main__':
    ...
    if not os.path.isdir('xyz'):
        os.mkdir("xyz")
        print("Directory xyz/ was not found, so it was created")
    # find storage energy
    compound_list = list()
    for idx, compound in data.iterrows():
        try: 
            ...
            compound_list.append({'rep': str(compound.comp_name),
                                  ...
                                  'prod': prod_qmconf,
                                  'storage': storage})
            reac_qmconf.write_xyz(to_file=True,path="xyz/")
            prod_qmconf.write_xyz(to_file=True,path="xyz/")
        except:
            pass
    ## find ts
